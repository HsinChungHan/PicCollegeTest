...
public final class PlaybackViewModel: ObservableObject {
    ...
    @Published public var barsInSelection: Int = 0            // 選取匡內 bar 總數
    @Published public var filledCountInSelection: Int = 0     // 已填充 bar 數（0-based 計數）

    private let clock: PlaybackClockUseCase
    private var bag = Set<AnyCancellable>()

    public init(clock: PlaybackClockUseCase = DefaultPlaybackClockUseCase()) {
        self.clock = clock
